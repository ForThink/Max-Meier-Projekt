...
fetch("/text?id=about", {
    ...
    if(response.status<400){
        response.json().then((json)=>{
            document.getElementById("aboutText").innerHTML=json.content
            document.getElementById("aboutEdit").value=json.content
        })
    }
})
if(admin){
    let adminbtn= document.getElementById("adminEdit");
    let editField = document.getElementById("aboutEdit");
    ...
    adminbtn.addEventListener("click", (ev)=>{
        ...
        if(edit){
            fetch("/text", {
                ...
                if(response.status<400){
                    document.getElementById("aboutText").innerHTML=editField.value
                }
            })
        }else{
