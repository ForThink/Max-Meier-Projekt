...
fetch("/text?id=help", {
    ...
    if(response.status<400){
        response.json().then((json)=>{
            document.getElementById("helpText").innerHTML=json.content
            document.getElementById("helpEdit").value=json.content
        })
    }
})
if(admin){
    let adminbtn= document.getElementById("adminEdit");
    let editField = document.getElementById("helpEdit");
    ...
    adminbtn.addEventListener("click", (ev)=>{
        ...
        if(edit){
            fetch("/text", {
                ...
                if(response.status<400){
                    document.getElementById("helpText").innerHTML=editField.value
                }
            })
        }else{
