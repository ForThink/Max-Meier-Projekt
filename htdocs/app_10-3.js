...
fetch("/text?id=agb", {
    ...
    if(response.status<400){
        response.json().then((json)=>{
            document.getElementById("agbText").innerHTML=json.content
            document.getElementById("agbEdit").value=json.content
        })
    }
})
if(admin){
    let adminbtn= document.getElementById("adminEdit");
    let editField = document.getElementById("agbEdit");
    ...
    adminbtn.addEventListener("click", (ev)=>{
        ...
        if(edit){
            fetch("/text", {
                ...
                if(response.status<400){
                    document.getElementById("agbText").innerHTML=editField.value
                }
            })
        }else{
